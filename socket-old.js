import localApp from './server.js';
// import remoteApp from './server.js';
// import mobileApp from './server.js';
...
const existsAsync = (path) => fs.existsSync(path);
...
// const wssServer = new WebSocketServer({ server: guestServer });
// const wsMobileServer = new WebSocketServer({ server: mobileServer }); // NEW
...
const GLOBAL_ROOM_ID = db.GLOBAL_ROOM_ID || 'global-room-00000000-0000-0000-0000-000000000000';
...
//MARK:startServer
async function startServer(localApp) {
    ...
    const localServer = http.createServer(localApp);
    const guestServer = http.createServer(remoteApp);
    const mobileServer = http.createServer(mobileApp);
    ...
    wsServer.setMaxListeners(0);
    wssServer.setMaxListeners(0);
    wsMobileServer.setMaxListeners(0);
    ...
    if (db.createDefaultGlobalRoom) {
        await db.createDefaultGlobalRoom();
    }
    if (db.migrateExistingDataToGlobalRoom) {
        await db.migrateExistingDataToGlobalRoom();
    }

    [hostKey, modKey] = generateServerKeys();
...
async function handleConnections(ws, type, request) {
    ...
    if (user) {
        ...
        const roleResult = await db.query('SELECT role FROM user_roles WHERE user_id = $1', [uuid]);
