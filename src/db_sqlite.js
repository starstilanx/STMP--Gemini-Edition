...
async function getTableData(tableName) {
    ...
    if (!schemaDictionary[tableName]) {
        logger.warn(`Attempted to query invalid table: ${tableName}`);
        throw new Error('Invalid table name');
    }

    logger.debug(`Getting data for table: ${tableName}`);
    const db = await dbPromise;
    try {
        // Safe to interpolate tableName here because we validated it against the schema keys
