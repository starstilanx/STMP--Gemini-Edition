...
async function startServer(localApp) {
    ...
    // Start the server
    localServer.listen(8181, '0.0.0.0', () => {
        logger.info('===========================');
        logger.info(`Host Server: ${color.yellow(`http://localhost:8181/`)}`);
        logger.info('===========================');
    });
...
if (parsedMessage.type === 'registerUser') {
    ...
    // Send response back to SAME socket
    socket.send(JSON.stringify({
        type: 'registerResponse',
        success: result.success,
        user: result.user, // or error: result.error
    }));
}
// 2. LOGIN
else if (parsedMessage.type === 'loginUser') {
    ...
    const result = await db.authenticateUser(username, password);
    if (result.success) {
        // Link this socket to the Authenticated User ID
        socket.data.uuid = result.user.user_id;
        socket.data.username = result.user.username;
        socket.data.role = result.user.role;

        // Broadcast presence or update lists...
    }
    socket.send(JSON.stringify({
        type: 'loginResponse',
        ...result
