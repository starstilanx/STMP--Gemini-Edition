...
async function writeUserChatMessage(userId, message, roomId = GLOBAL_ROOM_ID) {
    ...
    try {
        ...
        const insertMsgRes = await client.query(
            ...
            [userId, message, timestamp, session_id, roomId]
        );
...
async function registerUser(username, password, email = null) {
    ...
    try {
        ...
        await query('INSERT INTO user_roles (user_id, role) VALUES ($1, $2)', [user_id, 'user']);
...
async function getRoomById(roomId) {
    try {
        ...
        if (room) {
            ...
            if (room.settings) {
                // Postgres might auto-parse JSONB, but if it's text, parse it
                if (typeof room.settings === 'string') {
                    try {
                        room.settings = JSON.parse(room.settings);
                    } catch (e) {
                        logger.error(`[getRoomById] Failed to parse settings for room ${roomId}`, e);
                        room.settings = {};
                    }
                }
...
async function getSessionRoom(sessionId) {
    try {
        const res = await query('SELECT room_id FROM sessions WHERE session_id = $1', [sessionId]);
        return res.rows[0]?.room_id || null;
    } catch (err) {
        logger.error('Error getting session room:', err);
        return null;
    }
}
